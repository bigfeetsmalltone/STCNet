from model_test import Predictor
...
if __name__ == '__main__':
    ...
    if args.dataset == 'movingmnist':
        ...
        test_dataset = Satellite_Whole(args.test_data_dir, seq_len=args.short_len+args.out_len, train=False)
        testloader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, drop_last=False)
    ...
    with torch.no_grad():
        for test_i, test_data in enumerate(testloader):
            ...
            # frame prediction
            out_pred = pred_model(short_data, None, out_len = args.out_len, phase=2)
            ...
            #         if not os.path.isdir(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start)):
            #             os.makedirs(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start))
            #         for frame_i in range(args.short_len):
            #             cv2.imwrite(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start)+ '/input_'
            #                         +str(frame_i).zfill(5) + '.jpg', short_data[batch_i,frame_i,0,:,:].cpu().numpy()*255)
            #         for frame_i in range(args.out_len):
            #             cv2.imwrite(args.test_result_dir+'/satellite_'+str(video_i)+'_'+str(frame_start)+'/pred_'+
            ...
            if args.make_frame:
                for batch_i in range(batch_size_current):
                    ...
                    short_data = (short_data.detach().cpu().numpy())[:,:,:, 19:19+730,:]
                    out_pred = (out_pred.detach().cpu().numpy())[:,:,:, 19:19+730,:]
                    out_gt = (out_gt.detach().cpu().numpy())[:,:,:, 19:19+730,:]

                    if not os.path.isdir(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start)):
                        os.makedirs(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start) + '/input/')
                        os.makedirs(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start) + '/pred/')
                        os.makedirs(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start) + '/ground/')
                        os.makedirs(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start) + '/imsave/pred/')
                        os.makedirs(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start) + '/imsave/ground/')
                    for frame_i in range(args.short_len):
                        cv2.imwrite(args.test_result_dir + '/satellite_'+ str(video_i)+'_' + str(frame_start)+ '/input/'
                                    +str(frame_i) + '.jpg', short_data[batch_i,frame_i,0,:,:]*255)
                    for frame_i in range(args.out_len):
                        cv2.imwrite(args.test_result_dir+'/satellite_'+str(video_i)+'_'+str(frame_start)+'/pred/'+
                                    str(frame_i) +'.jpg', out_pred[batch_i,frame_i,0,:,:]*255)
                    for frame_i in range(args.out_len):
                        cv2.imwrite(args.test_result_dir+'/satellite_'+str(video_i)+'_'+str(frame_start)+'/ground/'+
                                    str(frame_i) +'.jpg', out_gt[batch_i,frame_i,0,:,:]*255)
                    for frame_i in range(args.out_len):
                        imsave(args.test_result_dir+'/satellite_'+str(video_i)+'_'+str(frame_start)+'/imsave/pred/'+
                                    str(frame_i) +'.jpg', out_pred[batch_i,frame_i,0,:,:]*255, cmap='gray')
                    for frame_i in range(args.out_len):
                        imsave(args.test_result_dir+'/satellite_'+str(video_i)+'_'+str(frame_start)+'/imsave/ground/'+
                                    str(frame_i) +'.jpg', out_gt[batch_i,frame_i,0,:,:]*255, cmap='gray')
